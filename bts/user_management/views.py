...
def modificar_user_per(request):
	...
	if u.has_perm('user_management.add_user'):
...
def modificar_user(request,user_iden):
	...
	if us.has_perm('user_management.add_user') and us.has_perm('user_management.delete_user'):
...
def detail_per(request):
	...
	session=Session.objects.get(session_key=request.session.session_key)
        uid=session.get_decoded().get('_auth_user_id')
    	us=User.objects.get(pk=uid)
	if us.has_perm('user_management.change_user'):
...
def eliminar_usuario(request,user_iden):
	session=Session.objects.get(session_key=request.session.session_key)
        uid=session.get_decoded().get('_auth_user_id')
    	us=User.objects.get(pk=uid)
	if us.has_perm('user_management.delete_user'):
